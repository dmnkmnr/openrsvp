...
```bash
docker run -d -p 8080:8080 -v openrsvp-data:/data -e BASE_URL=http://localhost:8080 ghcr.io/dmnkmnr/openrsvp:latest
```
...
```bash
git clone https://github.com/dmnkmnr/openrsvp.git
cd openrsvp
cp .env.example .env
docker compose up -d
```
...
services:
  openrsvp:
    image: ghcr.io/dmnkmnr/openrsvp:latest
