...
func (s *Store) Create(ctx context.Context, a *Attendee) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO attendees (id, event_id, name, email, phone, rsvp_status, rsvp_token, contact_method, dietary_notes, plus_ones, plus_ones_children, import_source, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		a.ID, a.EventID, a.Name, a.Email, a.Phone, a.RSVPStatus,
		a.RSVPToken, a.ContactMethod, a.DietaryNotes, a.PlusOnes, a.PlusOnesChildren, a.ImportSource, now, now,
	)
	if err != nil {
...
func (s *Store) FindByID(ctx context.Context, id string) (*Attendee, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, event_id, name, email, phone, rsvp_status, rsvp_token, contact_method, dietary_notes, plus_ones, plus_ones_children, import_source, created_at, updated_at
...
func (s *Store) Update(ctx context.Context, a *Attendee) error {
	...
	_, err := s.db.ExecContext(ctx,
		`UPDATE attendees SET name = ?, email = ?, phone = ?, rsvp_status = ?, contact_method = ?, dietary_notes = ?, plus_ones = ?, plus_ones_children = ?, updated_at = ?
		 WHERE id = ?`,
		a.Name, a.Email, a.Phone, a.RSVPStatus, a.ContactMethod, a.DietaryNotes, a.PlusOnes, a.PlusOnesChildren, now, a.ID,
	)
	if err != nil {
...
func (s *Store) GetStats(ctx context.Context, eventID string) (*RSVPStats, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT rsvp_status, COUNT(*), COALESCE(SUM(plus_ones), 0), COALESCE(SUM(plus_ones_children), 0) FROM attendees WHERE event_id = ? GROUP BY rsvp_status`, eventID,
	)
	if err != nil {
	...
	for rows.Next() {
		var status string
		var count, plusOnes, plusOnesChildren int
		if err := rows.Scan(&status, &count, &plusOnes, &plusOnesChildren); err != nil {
		...
		case "attending":
			...
			stats.AttendingChildren = plusOnesChildren
		case "maybe":
			stats.Maybe = count
...
func (s *Store) FindFirstWaitlisted(ctx context.Context, eventID string) (*Attendee, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, event_id, name, email, phone, rsvp_status, rsvp_token,
				contact_method, dietary_notes, plus_ones, plus_ones_children, import_source, created_at, updated_at
		 FROM attendees
...
func scanAttendee(row *sql.Row) (*Attendee, error) {
	...
	err := row.Scan(
		...
		&a.DietaryNotes, &a.PlusOnes, &a.PlusOnesChildren, &importSource, &createdAt, &updatedAt,
	)
	if err != nil {
