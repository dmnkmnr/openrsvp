...
const eventColumns = `id, organizer_id, title, description, event_date, end_date, location, timezone, retention_days, status, share_token, language, contact_requirement, map_provider, map_custom_url, show_headcount, show_guest_list, rsvp_deadline, max_capacity, waitlist_enabled, comments_enabled, series_id, series_index, series_override, created_at, updated_at`
...
func (s *Store) Create(ctx context.Context, e *Event) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		e.Location, e.Timezone, e.RetentionDays, e.Status, e.ShareToken, e.Language, e.ContactRequirement, e.MapProvider, e.MapCustomURL,
...
func (s *Store) Update(ctx context.Context, e *Event) error {
	...
	_, err := s.db.ExecContext(ctx,
		`UPDATE events SET title = ?, description = ?, event_date = ?, end_date = ?, location = ?, timezone = ?, retention_days = ?, status = ?, language = ?, contact_requirement = ?, map_provider = ?, map_custom_url = ?, show_headcount = ?, show_guest_list = ?, rsvp_deadline = ?, max_capacity = ?, waitlist_enabled = ?, comments_enabled = ?, series_id = ?, series_index = ?, series_override = ?, updated_at = ?
		 WHERE id = ?`,
		e.Title, e.Description, eventDate, endDate, e.Location, e.Timezone,
		e.RetentionDays, e.Status, e.Language, e.ContactRequirement, e.MapProvider, e.MapCustomURL, boolToInt(e.ShowHeadcount), boolToInt(e.ShowGuestList),
...
func scanEvent(row *sql.Row) (*Event, error) {
	...
	err := row.Scan(
		...
		&e.RetentionDays, &e.Status, &e.ShareToken, &e.Language, &e.ContactRequirement, &e.MapProvider, &e.MapCustomURL,
