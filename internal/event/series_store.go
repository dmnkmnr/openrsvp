...
func (s *SeriesStore) Create(ctx context.Context, es *EventSeries) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		es.MaxOccurrences, es.SeriesStatus, es.RetentionDays, es.Language, es.ContactRequirement, es.MapProvider, es.MapCustomURL,
...
func (s *SeriesStore) FindByID(ctx context.Context, id string) (*EventSeries, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, organizer_id, title, description, location, timezone, event_time, duration_minutes, recurrence_rule, recurrence_end, max_occurrences, series_status, retention_days, language, contact_requirement, map_provider, map_custom_url, show_headcount, show_guest_list, rsvp_deadline_offset_hours, max_capacity, created_at, updated_at
...
func (s *SeriesStore) Update(ctx context.Context, es *EventSeries) error {
	...
	_, err := s.db.ExecContext(ctx,
		`UPDATE event_series SET title = ?, description = ?, location = ?, timezone = ?, event_time = ?, duration_minutes = ?, recurrence_rule = ?, recurrence_end = ?, max_occurrences = ?, series_status = ?, retention_days = ?, language = ?, contact_requirement = ?, map_provider = ?, map_custom_url = ?, show_headcount = ?, show_guest_list = ?, rsvp_deadline_offset_hours = ?, max_capacity = ?, updated_at = ?
		 WHERE id = ?`,
...
func scanSeries(row *sql.Row) (*EventSeries, error) {
	...
	err := row.Scan(
		...
		&maxOccurrences, &es.SeriesStatus, &es.RetentionDays, &es.Language, &es.ContactRequirement, &es.MapProvider, &es.MapCustomURL,
